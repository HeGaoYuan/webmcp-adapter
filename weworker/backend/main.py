...
from models import ChatRequest, ChannelsSettingsUpdate, SettingsUpdate, WorkTitleUpdate
...
async def startup():
    await store.init()
    bridge.start()
    channel_manager.configure(get_settings(), agent, store)
...
async def get_status():
    return {
        ...
        "external_write": store.last_external_write,
        "channels": channel_manager.get_statuses(),
    }
...
async def get_channels_settings():
    ...
    return {
        "qq": {
            "enabled": qq.enabled,
            "app_id": qq.app_id,
            "has_app_secret": bool(qq.app_secret),
        }
    }


@app.post("/settings/channels")
async def post_channels_settings(body: ChannelsSettingsUpdate):
    current = get_settings()
    new_channels = current.channels.model_copy(deep=True)
    if body.qq:
        patch = {k: v for k, v in body.qq.model_dump().items() if v is not None}
        new_channels.qq = new_channels.qq.model_copy(update=patch)
    updated = update_settings({"channels": new_channels})
    # Hot-reload channels without app restart
    await channel_manager.stop()
    channel_manager.configure(updated, agent, store)
    await channel_manager.start()
    qq = updated.channels.qq
    return {"qq": {"enabled": qq.enabled, "app_id": qq.app_id, "has_app_secret": bool(qq.app_secret)}}
...
async def chat(work_id: str, req: ChatRequest):
    ...
    system_prompt = (
        "你是 WeWorker（微牛马），一个能通过 Chrome 扩展直接操作网页的 AI 助手。\n"
