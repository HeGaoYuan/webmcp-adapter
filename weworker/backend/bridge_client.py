    ...
    async def _handle_message(self, msg: dict):
        ...
        if msg_type == "tools_updated":
            ...
            if domains:
                ...
                # Flat format: tools_snapshot.tools = [{name, domain, tabCount, ...}, ...]
                for tool in msg.get("tools", []):
                    domain = tool.get("domain", "")
                    if domain not in self._tools:
                        self._tools[domain] = {"tools": [], "tabCount": tool.get("tabCount", 0)}
                    # Avoid duplicates
                    names = {t.get("name") for t in self._tools[domain]["tools"]}
                    if tool.get("name") not in names:
                        self._tools[domain]["tools"].append(tool)
            if self._tools:
                self._extension_connected = True
