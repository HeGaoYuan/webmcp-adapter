...
async function viewOrders() {
  try {
    ...
    return {
      status: 'success',
      message: 'Navigated to my orders and parsed order list',
      previousUrl: currentUrl,
      currentUrl: window.location.href,
      orderUrl: orderUrl,
      pageLoaded: document.readyState === 'complete',
...
async function buyNow() {
  try {
    ...
    if (iframe && iframe.src) {
      // 导航到 iframe 的 URL，这样就能直接操作结算页面了
      ...
      return {
        status: 'success',
        message: 'Clicked buy now button and navigated to checkout page',
        checkoutUrl: iframeUrl,
        note: 'Now on checkout page, you can call pay_now',
      };
    }

    return {
      status: 'success',
      message: 'Clicked buy now button',
      currentUrl: window.location.href,
      note: 'Checkout iframe should appear',
    };
  } catch (error) {
    return {
  ...
  window.__webmcpRegister({
    ...
    tools: [
...

      {
        ...
        description:
          'Click the "立即购买" (Buy Now) button on the current product page. Must be on a product page (item.jd.com). Use www.jd.com.open_product first to navigate to a product page. After clicking, a checkout iframe will appear.',
        parameters: { type: 'object', properties: {}, required: [] },
        handler: () => buyNow(),
      },
